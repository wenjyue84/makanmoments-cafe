...
export function AdminCategoriesPanel({
  ...
  // Load POS categories on first interaction
  ...
  async function deleteCategory(id: number, name: string) {
    if (
      !confirm(`Delete "${name}"? This will also remove it from all menu items.`)
    )
      return;
  ...
  return (
    ...
    >
      {/* ── POS Categories (read-only, synced from POS) ── */}
      <div className="rounded-xl border bg-white p-6">
        <div className="mb-1 flex items-center gap-2">
          <h2 className="text-lg font-semibold text-gray-900">POS Categories</h2>
          <span className="rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-500">
            Synced from FeedMe
          </span>
        </div>
        <p className="mb-4 text-xs text-gray-500">
          These categories come from your POS system. You can reorder or rename them here.
        </p>
        ...
        {!loaded ? (
          <p className="text-sm text-gray-500">Click anywhere to load categories…</p>
        ) : categories.length === 0 ? (
      ...
      <div className="rounded-xl border bg-white p-6">
        ...
        <p className="mb-4 text-xs text-gray-500">
          Curate items into website-only groups like "New Items" or "Vegetarian" — independent of POS.
        </p>
